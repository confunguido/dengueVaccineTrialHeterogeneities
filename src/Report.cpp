...
void Report::updateCohortReport(int currDay, Human * h){
	...
	for(unsigned int i = 0; i < health_outcomes.size(); i++){
	    string health_str = health_outcomes[i];
	    cohortData[cohortNum]["Vaccine"]["Seropositive"][health_str] = 0;
	    cohortData[cohortNum]["Vaccine"]["Seronegative"][health_str] = 0;
	    cohortData[cohortNum]["Placebo"]["Seropositive"][health_str] = 0;
	    cohortData[cohortNum]["Placebo"]["Seronegative"][health_str] = 0;
	}
    }
    string vac_status;
    string sero_status;
	...
	cohortData[cohortNum][vac_status][sero_status]["Size"]++;    
	h->setCohortCount(true);
    }

    if(h->infection != nullptr){
	if(floor(h->infection->getSymptomOnset()) == currDay){
	    cohortData[cohortNum][vac_status][sero_status]["Inf"]++;
	    if(h->isSymptomatic() == true){
		cohortData[cohortNum][vac_status][sero_status]["Dis"]++;		
		if(h->isHospitalized()){
		    cohortData[cohortNum][vac_status][sero_status]["Hosp"]++;
		    cohortData[cohortNum][vac_status][sero_status]["cumAgeOfHospitalization"] += h->getAgeDays(currDay);
		    if(h->isDeadFromInfection()){
			deathsFromInfection++;
			int le = h->getDeathday();
			if(le == -1){
			...
			cohortData[cohortNum][vac_status][sero_status]["discountedLifeExpectancy"] += le - currDay;
		    }
		}
	    }
	}
...
void Report::printCohortReport(int currDay, int vDay_){    
	...
	//unordered_map<int,unordered_map<string,unordered_map<string, unordered_map<string,int>>>> cohortData; // <Cohort # < vaccine status < serostatus at vac. <output name, output value>>>
	for(unsigned int i = 0; i < cohortData.size(); i++){
...
void Report::printCohortHeader(){
    outCohort << "Day,Cohort,Vaccine,Serostatus,PopSize,Infectious,Dis,Hosp,cumAgeOfHospitalization,discountedLifeExpectancy\n";
}
